import gradio as gr
import torch
from transformers import AutoTokenizer, AutoModelForCausalLM  # 🛠️ Typo fixed: CauselLM → CausalLM
...
# 🛠️ Fixed function signature and typos
def generate_response(prompt, max_length=512):
    ...
    with torch.no_grad():  # 🛠️ "With" → "with", no_grad() typo fixed
        outputs = model.generate(
            **inputs,
            max_length=max_length,
            temperature=0.7,
            do_sample=True,
            pad_token_id=tokenizer.eos_token_id  # 🛠️ Pad_token_id → pad_token_id
        )

    response = tokenizer.decode(outputs[0], skip_special_tokens=True)
    response = response.replace(prompt, "").strip()  # 🛠️ Fixed formatting
    return response
...
# 🛠️ Fixed indentation, capitalizations, and Python syntax for "with"
